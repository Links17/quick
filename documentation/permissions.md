...
| --- | --- | --- | --- | --- |
| Pasteboard | Read text/image | Required | Required | Used as translation or OCR source. |
| Keychain | Read/write API key | Required | Required | Stores provider key. |
| Network | Send source text | Required for text | Required for text | Sends text to configured Base URL. Image OCR stays local. |
...
- Quick should not send copied images to the network for OCR.
